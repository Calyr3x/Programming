...
enum {
    _short_int, _unsigned_short_int, _int, _unsigned_int, _float, _double
} DATA_TYPE;
...
Возвращает -1, если  x1 < x2
Возвращает 0, если  x1 == x2
Возвращает 1, если  x1 > x2.
*/
...
/*
Функция пересылки b в a.
*/
void move(void *a, void *b, int elemsize) {
    std::memcpy(a, b, elemsize);
}
...
template <typename T> void merge_sort(T *array, int n, int SORT_DIRECTION) {
    bool swapped = false;
...
/*
Функция выбора типа сортировки и её направления.
*/
template <typename T>
void sort(T *array, int size, int elemsize, int SORT_DIRECTION, int SORT_METHOD) {
    if(*array == 0) {
